...
class GrilleCalculator:
    ...
    def generate_inventory_df(data, pitch, stock_plan):
        ...
        for _, row in data.iterrows():
            ...
            for item in row_items:
                item["Quantity"] = item.get("Quantity", 0) * row["qty_areas"]
    ...
    def run(self):
        ...
        data[["num_L", "num_inverse_L"]] = data.apply(
            lambda r: pd.Series(GrilleCalculator.calculate_endcaps(r)),
            axis=1
        )
        ...
        offer_df_cols, offer_df = GrilleCalculator.generate_offer_df(data)
        inventory_df = GrilleCalculator.generate_inventory_df(data, self.pitch, out["stock_plan"])
